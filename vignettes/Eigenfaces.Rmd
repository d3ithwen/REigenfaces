...
A subset of the *lfwcrop_grey* data set is included in the package as `dataset` (for more information: `?dataset`). To load a different training data set and determine the eigenfaces use:

`my_dataset <- REigenfaces::load_dataset(path_to_data, pattern="(0001)|(0002)|(0003)|(0004)", max_images=13000, max_eigenfaces=100L)`
...
`my_dataset <- change_max_eigenfaces(my_dataset, max_eigenfaces=42L)`
...
n=16 similar faces in the training data set of image 2 can be displayed as follows:
```{r warning=FALSE}
show_similar_faces(dataset, dataset$images[2], 16)
```

If you want to display similar faces in the training data for a new face, you can do so using  
`image <- load_images(path_to_images, pattern="(0001)|(0002)|(0003)|(0004)", max_images=1L)` and
`show_similar_faces(dataset, image[1], 16)` afterwards.  
