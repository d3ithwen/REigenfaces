...
`my_dataset <- REigenfaces::load_dataset(path_to_data, pattern="(0001)|(0002)|(0003)|(0004)", max_eigenfaces=100L)`

where `path_to_data` is the path to the data as a `character` vector. `pattern` is an optional regular expression. Only file names which match the regular expression will be loaded. In our example data set, there are multiple images of the same person. By utilizing the pattern, only a maximum of four images of the same person are used. The number of eigenfaces that will be determined can be adjusted via `max_eigenfaces`. Since all major computations are performed here, this can take quite some time, depending on the size of your data.  
